...
# from app.models.requests import AnalyzeRequest, GenerateQueriesRequest
...
# async def generate_queries(document_id: str, request: GenerateQueriesRequest):
...
#         # Generate queries
#         queries = query_generator.generate_queries(document_text, request.main_query)
...
from fastapi import APIRouter, UploadFile, File, Form
from typing import Optional
from app.models.requests import GenerateQueriesRequest, BatchAnalyzeRequest
...
# ---------------------------------------------------------
# Upload document (stores text in Redis, no embeddings yet)
# ---------------------------------------------------------
@router.post("/documents", response_model=DocumentUploadResponse)
async def upload_document(
    file: UploadFile = File(...),
    ...
    try:
        ...
        if file_extension == "txt":
            ...
            if file_extension == "pdf":
                document_text = document_service.parse_pdf(path)
            else:
                document_text = document_service.parse_docx(path)
        ...
        preview = document_text[:200] + ("..." if len(document_text) > 200 else "")
...
# ---------------------------------------------------------
# Generate queries (Groq, no embeddings)
# ---------------------------------------------------------
@router.post(
    "/documents/{document_id}/generate-queries",
...
# ---------------------------------------------------------
# Batch RAG analysis (Gemini embeddings, cached)
# ---------------------------------------------------------
@router.post(
...
async def batch_analyze_document(document_id: str, request: BatchAnalyzeRequest):
    ...
    if not request.custom_queries or len(request.custom_queries) < 3:
        return BatchAnalyzeResponse(
            success=False,
            document_id=document_id,
            query_results=[],
            aggregate_winner="",
            aggregate_insights=[],
            message="Minimum 3 queries required",
        )

    queries = request.custom_queries[:10]
    document_text = document_storage.load_cached_text(document_id)
    ...
    formatted = [
        QueryResult(
            ...
            best_strategy=q["best_strategy"],
            best_accuracy=q["best_accuracy"],
            all_results=q["all_results"],
        )
    ...
    return BatchAnalyzeResponse(
        ...
        aggregate_winner=aggregate_winner,
        aggregate_insights=aggregate_insights,
        message=f"Successfully analyzed {len(formatted)} queries",
    )
